...
[~,unity_gain_idx] = min(abs(dbgain_ninv));

[~,margin_180] = min(abs(bode_ninv.Phase_Response1 - (bode_ninv.Phase_Response1(unity_gain_idx) - 180)));

freq_log = log10(bode_ninv.X__Trace1___AC_FREQUENCY_);

lin_fit = polyfit(freq_log(cutoff_freq_idx:end-5),dbgain_ninv(cutoff_freq_idx:end-5),1);
...
fprintf("Unity Gain frequency is %.2f and frequency of phase margin 0 is %.2f\n",bode_ninv.X__Trace1___AC_FREQUENCY_(unity_gain_idx),bode_ninv.X__Trace1___AC_FREQUENCY_(margin_180));

y1 = polyval(lin_fit,freq_log(cutoff_freq_idx:end-5));
...
plot(freq_log(cutoff_freq_idx:end-5),y1);
